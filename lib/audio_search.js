...
async function handleImage(absPath, metadata) {
	...
	if (fs.existsSync(manualFile)) {
		logger.log({
			level: 'info',
			message: `Using manual image ${manualFile} for audio file ${absPath}`,
		});
		const url = manualFile.replace(rootPath, `${httpRoot}audio/`);
		return { path: manualFile, url };
	}
	...
	if (fs.existsSync(absImagePath)) {
		logger.log({
			level: 'info',
			message: `Using precached image ${absImagePath} for audio file ${absPath}`,
		});
		const url = absImagePath.replace(rootPath, `${httpRoot}images/`);
		return { path: absImagePath, url };
	}
	...
	if (metadata.common.picture) {
		...
		logger.log({
			level: 'info',
			message: `Writing image ${absImagePath} for audio file ${absPath}`,
		});
		...
		return { path: absImagePath, url: absImageUrl };
	}
	return { path: '', url: '' };
}
